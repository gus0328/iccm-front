...
<script>
  export default {
    ...
    methods: {
      queryDevices() {
        this.loading = true;
        let url = "/work/wearDevice/pageList?pageNum=" + this.devicePageNo + "&pageSize=" + this.devicePageSize +
      ...
      editSave() {
        this.$refs.editForm.validate((valid) => {
          if (valid) {
            let url = "/work/wearDevice/edit";
            if (this.editTitle == "新增") {
              url = "/work/wearDevice/add";
            }
            this.$ajax.request({
              url: url,
              data: this.editForm,
